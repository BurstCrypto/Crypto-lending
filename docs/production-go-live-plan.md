...
| Authentication      | Migration `0025` and the application locally enforce bounded identity/session/CSRF HMAC overlap, provider/issuer/account-bound identity aliases, candidate-bound session/replay handling, and cross-version rate limiting. The zero-desired-count production task selects the pre-authentication key plus all six canonical authentication/wallet ring documents from one exact immutable Secrets Manager VersionId and forbids legacy or mutable-stage selectors. CREATE requires the exact version, and both current UPDATE intents preserve the deployed ARN/version/KMS tuple. No dedicated auth/wallet transition guard, Cognito tenant, populated external ring secret, KMS custody, egress, or deployed evidence exists.                                                                                                                                                            | Add a reviewed current-to-target auth/wallet version transition guard; obtain deployed non-production Cognito, secret/KMS custody and exact-version field-read evidence, staged rotation, task replacement, old-function denial, recovery/outage, and independent-review evidence                                             |
