...
2. Complete and activate the dormant read-only provider-position boundary.
   ...
   checked-in mainnet pair registry is empty and `NOT_APPROVED`, and no live
   source implementation or owner-authorized database writer exists. There is no
   populated evidence claim, runtime registration or activation, deployment, or
