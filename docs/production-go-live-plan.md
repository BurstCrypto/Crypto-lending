...
2. Complete and activate the dormant read-only provider-position boundary.
   ...
   approved policy binding, production pool-timeout binding, or production
   registration exists. A concrete Node deadline runner now exists as dormant
   source only: it propagates deadline cancellation, waits for started work to
   settle, and cleans up its timer and listener. The dormant coordinator sends
   ...
   deadline; production `connectionTimeoutMs` must not exceed the configured
   admission bound, and this source supplies no hard latency evidence.
...
| Provider positions        | Dormant contracts now preserve provider, protocol, market, position, position-kind, wallet, network, asset, source, and coverage identity. The coverage-aware reader v2 contract can return only a fingerprint-bound covered snapshot, so an empty set can mean zero only when every exact wallet × approved-market target is complete, agreeing, current, and declares zero positions. A concrete but unregistered Node deadline runner aborts the shared admission on bounded deadlines or failures, waits for started cooperative work to settle, and cleans up its timer/listener. The exact signal now reaches the PostgreSQL wallet-roster query while preserving the legacy unsigned path; non-cancellable pool acquisition may still extend drain beyond the logical deadline, and production pool timeout binding remains absent. Offline preflight pins a selected seventeen-file reader/trusted-assembly/deadline-runner/wallet-roster critical-source slice and rejects trust, timing, signal substitution, cancellation, cleanup, query fallback, authority, zero-target, or feature-surface drift within those files; it does not claim recursive dependency, whole-module-graph coverage, or hard latency. Conservative composition keeps liquid and supplied assets separate from borrow liabilities and preserves signed negative net positions. The local gate still reports all three expected feature registrations as missing; no ingestion adapter is registered, and production registration, policy approval binding, and wallet-balance composition runtime remain disabled. | Reviewed policy and approval bindings, live provider adapters, wallet-balance composition, and duplicate-asset/provider isolation evidence all pass for the shipped revision |
