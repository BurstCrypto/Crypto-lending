...
2. Complete and activate the dormant read-only provider-position boundary.
   ...
   approved policy binding, gate-consumed/deployed pool-timeout binding, or
   production registration exists. A concrete Node deadline runner now exists
   ...
   pool from that owned snapshot. Its returned options are not yet consumed by
   coordinator construction, so this is configuration ordering only, not a
   production binding or hard end-to-end latency evidence.
