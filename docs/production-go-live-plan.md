...
2. Complete and activate the dormant read-only provider-position boundary.
   ...
   observation identity, but no concrete durable-anchor reader,
   persistence admission, approved policy
   binding, deployed pool-timeout binding, or production registration exists. A
   ...
   injected reader methods once, verifies an unissued sentinel is rejected,
   ...
   identity. The dormant composition now accepts only an optional durable
   reader, constructs the assembler privately after its deadline runner and
   before its coordinator, and exposes neither object. It performs no ambient
   I/O, and no concrete durable-anchor reader exists.
   ...
   preserves migration `0028`'s worker suspension. There is no application
   reader/adapter, evidence writer/producer, populated evidence claim, runtime
   registration or activation, deployment, or live evidence. Construction
...
| Provider positions        | Dormant contracts now preserve provider, protocol, market, position, position-kind, wallet, network, asset, source, and coverage identity. The coverage-aware reader v3 accepts only account and correlation IDs and returns an exact frozen envelope containing the server-authored parser time and exact covered-snapshot identity; callers cannot supply evaluation time or access raw admission, verifier, lifecycle, persistence, or financial-action capabilities. An empty set can mean zero only when every exact wallet × approved-market target is complete, agreeing, current, and declares zero positions. The private frozen null-prototype reader invokes only the composition's captured `admitAndAssemble` method. A concrete dormant trusted-chain-assessment assembler now sits behind an exact Ethereum/Solana-only durable-anchor reader port; it authenticates each opaque result before inspection, rejects request and capability clones, reads every target sequentially, and privately seals exact request and verification-context identity. Migration `0029` adds append-only global Ethereum/Solana anchor evidence and invalidation/quarantine controls to that pinned slice without account or wallet PII columns; only its active-chain-bound-wallet-gated API read has a runtime grant, while record/control operations remain owner-only. It still has no concrete application durable-anchor reader/adapter, evidence writer/producer, populated evidence rows, ambient I/O, production registration, deployment, or live evidence. A concrete but unregistered Node deadline runner aborts the shared admission on bounded deadlines or failures, waits for started cooperative work to settle, and cleans up its timer/listener. The exact signal now reaches the PostgreSQL wallet-roster query while preserving the legacy unsigned path; non-cancellable pool acquisition may still extend drain beyond the logical deadline. A dormant runtime-budget resource directly constructs a lazy API pool from owned immutable configuration only after proving `connectionTimeoutMs <= deadlineMilliseconds <= 30_000` and concurrency from 1 through 8. The dormant composition gate-consumes that resource's exact pool and frozen admission options through the concrete wallet-roster/deadline/private-assembler/coordinator graph, rolls back owned construction, and closes by aborting admission before PostgreSQL and admitted-call drain and final pool shutdown. A non-cooperative provider can still delay physical drain indefinitely; no deployed binding or hard end-to-end latency evidence exists. Offline preflight pins a selected twenty-six-file reader/trusted-assembly/durable-anchor/launch-network-policy/deadline-runner/wallet-roster/runtime-budget/runtime-composition critical-source slice and rejects trust, timing, source-method substitution, active-controller lifecycle, signal substitution, cancellation, cleanup, query fallback, budget/config substitution, raw-assembly or private-assembler bypass, facade exposure, authority, zero-target, or feature-surface drift within those files; it does not claim recursive dependency or whole-module-graph coverage. Conservative composition keeps liquid and supplied assets separate from borrow liabilities and preserves signed negative net positions. The local gate still reports `PROVIDER_POSITION_READER_FEATURE_REGISTRATION_MISSING`, `PROVIDER_POSITION_TRUSTED_ASSESSMENT_FEATURE_REGISTRATION_MISSING`, and `PROVIDER_POSITION_DEADLINE_RUNNER_FEATURE_REGISTRATION_MISSING`; the dormant assembler does not clear the trusted-assessment blocker because it has no concrete durable-anchor reader, its private wiring is not registered, and it has no deployed live evidence. No ingestion adapter is registered, and production registration, policy approval binding, and wallet-balance composition runtime remain disabled. | Reviewed policy and approval bindings, live provider adapters, wallet-balance composition, and duplicate-asset/provider isolation evidence all pass for the shipped revision                                                                                                                                                  |
...
### Cumulative `0018` through `0029` database rollout

Migrations `0018` through `0029` continue the exact, cumulative principal
verifier chain. They must ship as one coordinated maintenance-window cutover
with an application artifact that expects `0029`; they are not compatible with
...
Quiesce old API writes, apply all twelve migrations with the migration principal,
then require repeated `0029` readiness checks through both API and worker
...
or control row exists. Any schema rollback also requires an exactly compatible
