...
supplied approval list, and inventory commit `040e93a` byte-pins it and its
focused hostile-path specification. It has no
authenticated transcript acquisition, independent source, endpoint, clock,
database, runtime registration, or authority and explicitly leaves completeness
unestablished. It is not a seventh complete provider-position source and does
not change the 0-of-10 live-read or transaction-evidence counts.

Six account-scoped provider-position source artifacts now sit behind those
market-level transcript boundaries: Aave V3 Ethereum (`1538ec7`), Kamino Solana
(`1e70d80`), Compound III Ethereum (`7e10077`), SparkLend Ethereum (`d684428`),
Morpho Blue Ethereum (`5910ebf`), and Euler V2 Ethereum (`d478f3b`). They are
endpoint-free, direct-import-only, dormant, unregistered, and depend on injected
authenticated durable wallet/continuity context plus bounded finalized
transcripts. The Aave, Kamino, Compound, and Spark preflight pins grew the
selected local critical slice from its historical 38 artifacts to 42.
Registration commit `7617800` adds an inert reader-v3 facade backed by an empty,
`NOT_APPROVED`, and `DISABLED` Ethereum/Solana activation registry; preflight
commit `a684735` pins it as the 43rd artifact. Morpho and Euler source commits
`5910ebf` and `d478f3b`, inventory pin `a77a57e`, and preflight commit `5550fbf`
bring the reviewed boundary to 45 artifacts; all 85 focused preflight cases pass.
The incomplete Gearbox evaluator is deliberately outside that six-source,
45-artifact provider-position slice and is owned by its dedicated inventory
validator. None of these sources adds a live endpoint,
...
runtime input, and all 403 current lowercase TypeScript/TSX sources not excluded
...
current source total is 6,704,387 bytes and the reviewed aggregate pins SHA-256
...
### Dormant mainnet financial-action persistence boundary

Commit `ee9204d` adds migration `0033`, an owner-only append-only lifecycle for
exact Ethereum/Solana `SUPPLY` and `WITHDRAW` intents, signed-submission binding,
ambiguous broadcast observation, and reconciliation. It stores canonical public
chain transaction identity while retaining only digests for private payload,
signature, and evidence material. Prepare and bind require an active wallet and
unexpired intent; after binding, wallet revocation, operation drift, or expiry
cannot strand broadcast/reconciliation evidence, and reconciliation can recover
directly from the signed-bound state after a crash. Automatic resend, fee
escalation, API signing/broadcast, ledger settlement, and cross-chain authority
remain false.

Commit `a387124` adds the dedicated fail-closed validator that pins `0033`, its
specification, the exact migration-index tail, CLMA-FP-1 golden vectors,
owner-only ACLs, and runtime absence. Commit `a61bb57` adds three disposable
PostgreSQL 16 integration cases covering the test-schema migration chain through
`0033` plus its isolated-schema verifier, rolled-back reconciliation-index/
normalized-check tamper rejection, and SQLSTATE `22023` for a null expected
revision with zero retained history. Commit `5976d32` corrects initial
signed-submission binding and adds a guarded transition case that prepares and
binds, then revokes the wallet, fails the linked yield operation, and directly
reconciles `UNKNOWN` with no broadcast event, resend, or ledger authority.

Commits `b7289ac` and `b0b39d9` add a durable application port, a concrete
PostgreSQL adapter, its focused unit specification, and a guarded loopback
integration specification. They remain direct-import-only, undecorated,
unregistered, and ungranted. The adapter restricts requests to active USDC,
USDT, and PYUSD identities on Ethereum and Solana mainnet and exactly six
Ethereum plus four Solana provider/protocol bindings; those ten bindings remain
dormant and 0/10 are live. Each database method validates before I/O and issues
at most one fixed migration-`0033` function call, with no loop or retry. Any
throw, abort, or malformed row after dispatch becomes
`DATABASE_OUTCOME_UNKNOWN`; post-wallet ambiguity is
`READ_THEN_RECONCILE_ONLY`, never resend.
...
rejects unauthenticated terminal reconciliation. Deep-reorg and
authority-controlled results quarantine without rewriting terminal history or
authorizing a ledger reversal, resend, settlement, signing, or broadcast.

The local implementation now includes database prerequisite readers, Ethereum
and Solana signed-command verification, digest-only proof binding, wallet
revocation/key-rotation recovery, and a durable PostgreSQL two-queue scheduler
with fenced claims and authenticated completion. Migrations `0033` through
`0038` are registered; `0039` through `0042` remain direct-import-only and
unregistered. The source audit verifies this boundary and guarded PostgreSQL
tests exercise the dormant adapters.

API/worker composition, database grants, authority-row population, two approved
independent live sources, provider credentials and approved write manifests,
provider/legal/security/finance decisions, release-bound deployed/live evidence,
and downstream quarantine/remediation policy remain activation gates. No
signer, broadcaster, mainnet transaction construction, settlement authority, or
live financial-action route is enabled. The repository completion record is
[the zero-cost audit](zero-cost-repository-audit-2026-09-08.md).
...
| Mainnet writes            | The dormant Ethereum/Solana lifecycle, authenticated-finality and prerequisite adapters, exact signed-command verifiers and proof binder, wallet revocation/key-rotation recovery, and fenced PostgreSQL two-queue scheduler are implemented and locally audited. Migrations `0033`-`0038` are registered; `0039`-`0042` remain unregistered and owner-only. The adapters have no runtime wiring or grants; source/deployment authorities and write manifests remain empty. Live sources, scheduled execution, signing, broadcasting, settlement, operational recovery evidence, and independent approvals remain activation gates. All ten providers remain 0/10 live.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Every additional write gate in `mainnet-rollout.md` and the per-provider transaction evidence above passes for an exact allowlisted action                                                                                                                                                                                    |
...
The first subsequent production engineering slice is the
[production network foundation](production-network-foundation.md). Its standalone
template defines isolated network resources and dedicated workload security
groups, all disabled by default. Production data/services, workload activation,
deployment execution, and independent deployed evidence remain outstanding.
...
### Cumulative `0018` through `0035` database rollout

Migrations `0018` through `0035` continue the exact, cumulative principal
verifier chain. They must ship as one coordinated maintenance-window cutover
with an application artifact that expects `0035`; they are not compatible with
...
'hex')` separators, retains all 51 generated JSON string-type gates, and
    does not awaken or replace V1; and
16. dormant owner-only append-only mainnet financial-action intent, event, and
    global evidence-digest-claim state with exact prepare, submission-bind,
    broadcast-observation, reconciliation, and account-scoped read functions
    (`0033`). The migration persists canonical public Ethereum transaction hashes
    or Solana signatures but only digests for private payload/signature/evidence
    material, permits restart reconciliation directly after submission binding,
    grants no application principal, and authorizes no financial action;
17. address-bound V2 preparation that derives the accepted HMAC identity set
    from the current active, nonrevoked registered Ethereum/Solana wallet while
    preserving the immutable registration-era audit anchor (`0034`); and
18. empty exact source/deployment authority registries, append-only authenticated
    reconciliation admissions and authority controls, a chained post-finality
    safety overlay, effective-state reads, and a deferred rejection of legacy
    terminal reconciliation without a matching admission (`0035`). Every
    mutation and result keeps write, resend, signing, broadcast, and ledger-
...
migrations with the migration principal. Run repeated `0035` verifier checks
through schema-owner or migration verification authority, then separately prove
API and worker database connectivity plus the expected denial of owner-only
`0033` through `0035` functions before serving traffic. Register only the new API
and the existing reviewed outbox
publisher after those checks pass; drain all older tasks immediately.
...
Do not populate `0035` authority rows or grant, register, deploy, schedule, or
invoke any of these dormant components during this cutover; no owner-authorized
runtime binding exists. Neither `0030` through `0035` nor this rollout claims
physical commit acknowledgement.
...
relations; never delete retained safety history to force it.
...
owner and module wiring; that 43-artifact slice passed all 81 focused cases.
Morpho/Euler source commits `5910ebf` and `d478f3b`, inventory commit `a77a57e`,
...
The original `0033` integration suite has three schema/verifier-control cases
for the test-schema migration chain through `0033` plus its isolated-schema
verifier, rolled-back reconciliation-index/normalized-check tamper rejection,
and SQLSTATE `22023` rejection of a null expected revision with zero history.
The `5976d32` correction adds one guarded lifecycle transition case, and
`b0b39d9` adds one guarded integration case through the concrete adapter. Those
unique-schema loopback PostgreSQL 16 cases cover prepare, signed-submission bind,
wallet revocation and yield-operation failure, direct `UNKNOWN` reconciliation
without a broadcast record, result review, readback, and the absence of resend,
...
The authenticated-finality source contract and producer in `bae3116` and
`3e3ce75` pass 28 focused cases covering exact two-source and prerequisite
bindings, Ethereum/Solana finality and reorg rules, dense inputs, earliest
expiry, abort cleanup, unsupported-action rejection, and false authority. The
sidecar in `e7e27b7` passes 23 focused cases covering exact 24/25/2-argument SQL
dispatch, producer-candidate revalidation, one-shot cursors, ambiguous-write
recovery, and recorded-versus-current review identity. Migration `0035` in
`4f0a79f` passes 10 focused unit cases, the impacted 9-suite/88-case migration
set, and 6 guarded loopback PostgreSQL 16 cases. The database cases cover both
launch chains, legacy-bypass rejection, concurrent exact replay, authority
control, historical review replay with current state, sticky deep-reorg
quarantine, owner-only ACLs, and representative catalog tampering. API
typecheck, focused lint, formatting, and migration diff checks also pass. This
is local contract/database evidence only: it does not authenticate a live
source, populate an authority registry, grant a runtime, or approve a mainnet
write.
