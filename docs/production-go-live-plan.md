...
2. Complete and activate the dormant read-only provider-position boundary.
   ...
   resource's exact frozen options to coordinator construction. It performs no
   query or provider call while constructing, remains unregistered, and rolls
   back its owned pool if downstream construction fails. Close seals and aborts
