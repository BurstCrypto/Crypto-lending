...
2. Complete and activate the dormant read-only provider-position boundary.
   ...
   approved policy binding, cancellable wallet-roster database propagation, or
   ...
   started work to settle, and cleans up its timer and listener. The dormant
   coordinator sends one abort signal to every provider read, stops queued work
   on first failure, drains started cooperative reads, and closes the signal on
   every terminal path. The runner cannot physically stop a non-cooperative
   operation; in particular, the current wallet-roster database port cannot yet
   receive its signal.
