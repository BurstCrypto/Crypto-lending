...
2. Complete and activate the dormant read-only provider-position boundary.
   ...
   agreed finalized heads do not prove the selected candidate anchor finalized;
   the artifact remains `PROVISIONAL`, `DISPLAY_ONLY`, and
   `mayAuthorizeFinancialAction: false` pending a separate finalization gate. Its
   concrete PostgreSQL recorder authenticates that producer capability before
   and after exactly one cancellable call, preserves the exact signal, SQL casts,
   and 23-value order, strictly accepts one exact three-column result row, applies
   database-time/deadline/approval/network-freshness checks, and issues only an
   opaque frozen receipt bound to the original request. It has no generic-query
   fallback or automatic retry. The producer's 30-second bound is against its
   private `evaluatedAt`; the recorder correctly does not infer the bound from
   `deadlineAt - observedAt`. That recorder still targets `0029`; it cannot
   commit after `0030` and is not a usable migration-compatible writer. The
   checked-in mainnet pair registry is empty and
   `NOT_APPROVED`, and no live source implementation or owner-authorized recorder
   ...
   The existing recorder has no durable one-shot pre-dispatch intent or restart
   recovery and does not call `0030`; activation still requires the versioned
   `0031` recorder adaptation and reconciliation-only unknown-outcome workflow.
...
### Dormant provider-position record-deadline boundary
...
These are local schema guarantees only. Migration `0030` adds no runtime grant,
registration, deployment, populated row, or live evidence. The existing
recorder still targets `0029` and therefore cannot commit once `0030` is
installed. It has no durable one-shot pre-dispatch intent or restart recovery,
and physical commit acknowledgement cannot be guaranteed across a rejected
query or broken connection. A versioned `0031` recorder adaptation, durable
intent state, reconciliation-only unknown-outcome/restart recovery, and reviewed
commit-acknowledgement handling remain blockers. The local report must continue
to emit `PROVIDER_POSITION_READER_FEATURE_REGISTRATION_MISSING`,
`PROVIDER_POSITION_TRUSTED_ASSESSMENT_FEATURE_REGISTRATION_MISSING`, and
`PROVIDER_POSITION_DEADLINE_RUNNER_FEATURE_REGISTRATION_MISSING`; its
`liveReadEvidenceBound` and `transactionEvidenceBound` counts both remain zero.
...
### Cumulative `0018` through `0030` database rollout

Migrations `0018` through `0030` continue the exact, cumulative principal
verifier chain. They must ship as one coordinated maintenance-window cutover
with an application artifact that expects `0030`; they are not compatible with
...
Quiesce old API writes, apply all thirteen migrations with the migration principal,
then require repeated `0030` readiness checks through both API and worker
...
The checked-in provider-position recorder is not that compatible writer: it
still invokes `0029` directly and cannot commit after `0030`. Do not grant,
register, deploy, or invoke it during this cutover. A separately reviewed `0031`
adaptation must durably persist one-shot intent before dispatch and recover every
unknown or restarted operation with `RECONCILE_ONLY`, never a second `RECORD`.
Neither `0030` nor this rollout claims physical commit acknowledgement.
