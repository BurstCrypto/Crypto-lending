...
2. Complete and activate the dormant read-only provider-position boundary.
   ...
   preserves migration `0028`'s worker suspension. A dormant authority-free
   ...
   `deadlineAt - observedAt`. The checked-in mainnet pair registry is empty and
   ...
   Migration `0029` does not receive the producer deadline in its record function,
   so rejecting a late returned timestamp cannot roll back an already committed
   record. Query rejection or connection interruption can leave an unknown commit
   outcome. Function idempotence is not automatic retry or durable reconciliation;
   activation still requires database-atomic producer-deadline enforcement and a
