...
ephemeral OIDC public-client PKCE/authorization transaction secrets; OIDC identity HMAC; pre-authentication seal,
...
authentication and wallet key-ring documents without provisioning or reading
...
by default at zero desired tasks. The main open items are deployed managed
...
| Secrets      | History-aware source scanning, API/worker/Next.js Node error-path log allowlists, three exact reviewed outbox contracts, workload-specific static boundaries, fail-closed production key-ring selectors, and local bounded authentication and wallet rotation controls                           | Authorized external secret staging, deployed framework/runtime evidence, key custody, live rotation/revocation, database-bypass review, third-party handling, and full-hop transport review.                              |
