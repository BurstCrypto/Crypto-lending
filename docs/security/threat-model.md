...
flowchart LR
  ...
  FinalitySources[Future approved independent finality sources - absent] -. authenticated attestations .-> FinalityProducer[Dormant finality evidence producer]
  FinalityProducer --> FinalitySidecar[Owner-only unregistered authenticated-finality sidecar]
  FinalitySidecar --> DB
...
- authenticated-finality evidence crosses an opaque prerequisite capability,
  exactly two distinct source capabilities, and an owner-only sidecar boundary.
  The local producer and schema are dormant: no concrete prerequisite or source
  adapter is registered, source and deployment authority tables start empty,
  and every signing, broadcast, resend, settlement, and ledger-authority flag is
  false. Post-finality quarantine is a safety overlay and never rewrites terminal
  lifecycle history or authorizes or reverses ledger settlement;
...
- its dormant two-source coordinator revalidates each evidence fingerprint and
  ...
  runtime use;
- a separate dormant Ethereum/Solana authenticated-finality producer accepts
  only `SUPPLY` and `WITHDRAW`, exact action and deployment bindings, and exactly
  two independently captured primary/corroborating source attestations. It
  stops awaiting at the earliest authenticated deadline or expiry, capped at
  30 seconds, but cannot physically cancel a source implementation that violates
  its `AbortSignal` contract. The direct-import-only sidecar uses frozen requests,
  producer-issued capabilities, a same-instance one-shot database cursor, one
  fixed call, no automatic retry, and read-only recovery after ambiguity.
  Migration 0035 keeps source/deployment authority tables empty by default and
  stores append-only authority controls, authenticated admissions, and
  post-finality reviews under owner-only functions. Stored rows contain bounded
  digests and public chain identities, not raw signed payloads, signatures, or
  attestations. Deep-reorganization quarantine is sticky but never reverses the
  ledger or changes migration-0033 terminal history. There is no live source,
  prerequisite adapter, runtime scheduler, signing path, or settlement path;
...
| Ledger, lifecycle, idempotency, and external evidence                   | Append-only and intentionally has no general deletion path; migration 0035 extends this to authenticated-finality authority controls, admissions, and reviews, and refuses rollback after any such data exists                                                                                                                     | Finance, Legal, Risk, Privacy, and Records must approve authoritative retention, archive, legal hold, close, and tamper-evidence policy.                                           |
...
| Ledger       | Append-only journal controls, deterministic conservative valuation, strict corroboration-only parsers, exact provider-position coverage, full-state two-source comparison, guarded finalized-checkpoint recovery, and a closed provider-evidence carrier are locally enforced. The owner-only Ethereum/Solana lifecycle now has a dormant two-source authenticated-finality producer, direct-import-only sidecar, deferred admission constraint, and append-only post-finality safety overlay. Quarantine never changes terminal history or authorizes or reverses ledger settlement. All authority tables, runtime grants, signing, broadcast, settlement, and provider egress remain empty or disabled. | Privileged tamper evidence, approved concrete prerequisite and independent source adapters, controlled authority population, durable depeg-latch authorization, exact transaction payload/signature verification, unresolved-work claim/lease processing, runtime reconciliation/review scheduling, non-cooperative source containment, downstream quarantined-action remediation policy, deployment anchoring, and live finality/reorganization exercises. |
...
| Availability | Bounded queue, parser, logging, rate-limit, retry, and DLQ primitives; templates add queue-age/DLQ alarms, exact ALARM/OK notification binding, and an operational dashboard; dormant smart-lending and authenticated-finality reads add bounded deadlines and abort contracts, while explicitly retaining capacity risk when an implementation ignores cancellation                                                                                                                                                                                                                                                                                                                                      | Authorized topic/subscriptions, escalation ownership, per-account/global request and spend budgets, circuit breakers, caching, containment of non-cooperative finality sources, plus deployed queue/vendor/edge/load exercises.                                                                                                                                                                                                                             |
