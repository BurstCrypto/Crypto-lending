...
| RDS-managed master secret                                               | RDS owns the secret and ties its ARN and deletion lifecycle to the database; the template retains no independent copy. A closed nested schema-v1 lifecycle record and supporting-capture digest covered by the outer two-role-signed schema-v2 bundle, plus a fail-closed preflight blocker, exist locally                         | Acceptable deployed rotation, replacement, snapshot-restore, ARN-rebinding, old-password-denial, capture retention/matching, and cleanup evidence remains absent. |
...
two-role-signed schema-v2 bundle and binds the primary and restored database,
...
| Secrets      | History-aware source scanning, API/worker/Next.js Node error-path log allowlists, three exact reviewed outbox contracts, workload-specific static boundaries, one immutable shared-secret VersionId across all seven auth/wallet selectors, a two-role-signed deployment-integrated auth/wallet transition guard, schema-v3 histories for six A/B slots plus the Redis operator, a dedicated two-role-signed deployment-integrated Redis-operator transition guard, an RDS-managed KMS-bound master credential excluded from tasks, and a two-role-signed fail-closed RDS lifecycle/capture-digest gate | Authorized external secret staging, production auth/wallet and Redis-operator trust anchors and independently signed records, deployed framework/runtime evidence, acceptable RDS master rotation/restore proof and independently matched capture, key custody, live rotation/revocation and candidate/old-credential denial, database-bypass review, third-party handling, and full-hop transport review. |
| Supply chain | Lockfiles, pinned actions, local validation, a byte-inventory manifest, schema-v2 deployment-target binding whose v2 digest includes exact stack/RDS/KMS identities, and role-separated evidence/decision formats                                                                                                                                                                                                                                                                                                                                                                                       | Close the manifest filesystem/CI review, then obtain independent dependency/image review, external role decisions, final-candidate provenance, deployment digest verification, and revocation evidence.                                                                                                                                                                                                    |
