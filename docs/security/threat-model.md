...
- a separate dormant Ethereum/Solana authenticated-finality producer accepts
  ...
  ledger or changes migration-0033 terminal history. Database prerequisite
  readers, offline signed-command verification, digest-only proof binding,
  revocation/key-rotation recovery, and a durable fenced two-queue scheduler
  are implemented and tested locally. Migrations 0039-0042 and the adapters
  remain unregistered, with no runtime grants. Live sources, scheduled runtime
  execution, signing, and settlement remain disabled;
...
| Ledger       | Atomic journals, idempotency and transactional outbox; dormant owner-only lifecycle, authenticated-finality, exact signed-command proof, wallet recovery and durable fenced scheduler preserve immutable history and bounded recovery.                                                                                                                                                                                                                                                             | Independently review and operationalize the dormant adapters, approve live sources and exact write manifests, and define downstream quarantine/remediation without rewriting terminal history or reversing settlement.                                                                                                                                                                   |
