...
| Secrets      | History-aware source scanning, API/worker/Next.js Node error-path log allowlists, three exact reviewed outbox contracts, workload-specific static boundaries, one immutable shared-secret VersionId across all seven production key selectors, and local bounded authentication and wallet rotation controls | Authorized external secret staging, a dedicated auth/wallet VersionId transition guard, deployed framework/runtime evidence, key custody, live rotation/revocation, database-bypass review, third-party handling, and full-hop transport review. |
