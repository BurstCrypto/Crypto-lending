...
| Ledger       | Append-only journal controls, deterministic conservative valuation, strict corroboration-only parsers, exact provider-position coverage, full-state two-source comparison, guarded finalized-checkpoint recovery, and a closed provider-evidence carrier are locally enforced. An owner-only Ethereum/Solana action lifecycle plus direct-import-only adapter now preserve replay, ambiguous database outcomes, and reconciliation state; address-bound V2 proves current registered-wallet identity before preparation. All runtime grants, signing, broadcast, settlement, and provider egress remain disabled. | Privileged tamper evidence, approved independent live readers, durable depeg-latch authorization, exact transaction payload/signature verification, unresolved-work claim/lease processing, authenticated finality admission, append-only post-finality reorg handling, deployment anchoring, and live exercises.                                                                        |
