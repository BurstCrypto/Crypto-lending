...
| Secrets      | History-aware source scanning, API/worker/Next.js Node error-path log allowlists, three exact reviewed outbox contracts, workload-specific static boundaries, one immutable shared-secret VersionId across all seven auth/wallet selectors, and seven exact Redis/database credential bindings after inert adoption | Authorized external secret staging, dedicated auth/wallet and Redis-operator VersionId transition guards, deployed framework/runtime evidence, key custody, live rotation/revocation, database-bypass review, third-party handling, and full-hop transport review. |
