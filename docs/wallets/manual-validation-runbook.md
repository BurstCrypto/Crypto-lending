...
| C03 Connect                | Approve the intended test account and network                                                                        | Exact wallet, chain-qualified account, and connector are shown/stored                                                                                                                   |
| C04 Explicit restore       | Connect, reload, confirm there is no automatic reconnect, select the same connector, then click **Restore selected** | `reconnectOnMount` remains false; nothing restores before the click; only the selected connector and an allowed testnet session can restore without creating application authentication |
...
| C06 Account switch         | Select a different account in the wallet                                                                             | Only that connection updates; stale authorization is invalidated; no `accounts[0]` assumption                                                                                           |
...
| C18 Concurrent EVM wallets | Attempt to keep two independent EVM connector sessions active                                                        | **Deferred / unsupported:** the current harness does not claim simultaneous independent EVM sessions or cross-session isolation; retain as a future adapter/conformance case            |
...
| MM04 | D1/P1                      | Reconnect and choose another account                                                           | Explicitly selected account is used even if the provider returns several accounts             |
...
| CB07 | D1 — deferred              | Attempt simultaneous independent Coinbase and MetaMask sessions, then change one account/chain                                                                                                          | **Deferred / unsupported:** the current harness does not maintain two independent EVM sessions; no isolation result may be claimed                                                                                               |
