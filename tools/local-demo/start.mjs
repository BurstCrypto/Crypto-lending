...
process.stdout.write(
  [
    ...
    'The managed model stays local. The optional application proof deposits exactly 0.01 native SOL through one fixed Save/Solend route on Solana Devnet.',
    'One explicitly selected Wallet Standard wallet confirms and signs the exact six-instruction core; Phantom may prepend only the bounded compute-budget pair disclosed in the review.',
    'Use only free, valueless Devnet SOL from https://faucet.solana.com/. No EVM testnet funding or token approval is required; the provider is visible on-chain.',
    'The authenticated API validates and binds the signed bytes before one fixed Devnet broadcast attempt and never automatically resends them. It never signs or requests an airdrop. Completion requires finalized core-message, bounded compute-budget, and position verification; the proof position remains deposited because no withdrawal is included.',
